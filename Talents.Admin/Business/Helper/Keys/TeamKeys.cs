namespace Business.Helper
{

	public static class TeamKeys
	{

		public const string FuddTid = "辅导的梯队";
		public const string TidXinx = "梯队信息";
		public const string DaijJih = "带教计划";
		public const string DaijJih_Memo1 = "带教计划.培养目标";
		public const string DaijJih_Memo2 = "带教计划.具体计划";
		public const string DaijJih_Memo3 = "带教计划.带教小结";
		public const string DaijHuod_XueydChengzFangx = "带教计划.学员的成长方向";
		public const string TidChengy = "梯队成员";
		public const string DaijHuod = "带教活动";
		public const string DaijHuod_Timeline = "带教活动.时间轴";
		public const string DaijHuod_XueyChengg = "带教活动.学员成果";
		public const string DaijHuod_Edit = "带教活动.编辑";
		public const string DaijHuod_Delete = "带教活动.删除";
		public const string DaijHuod_HuodNeir = "带教活动.活动内容";
		public const string KecShis = "定向性课程实施";
		public const string DaijChengg = "带教成果";
		public const string KecShis_Bianj = "定向性课程实施.编辑";
		public const string KecShis_Anp = "定向性课程实施.安排";
		public const string KecShis_Chak = "定向性课程实施.查看";
		public const string KecShis_Delete = "定向性课程实施.删除";

		public const string TuanDXinx = "团队信息";
		public const string TuanDZhidJians = "制度建设和规划";
		public const string TuanDXiangm = "团队项目";
		public const string YanxHuod = "研修活动";
		public const string YanxHuod_Edit = "研修活动.编辑";
		public const string YanxHuod_Delete = "研修活动.删除";
		public const string YanxHuod_XueyChengg = "研修活动.学员成果";
		public const string TuanDGerXinx = "团队个人信息";
      public const string TuanDGerJh = "团队个人计划";
      public const string TuanDZiXiangm = "团队子项目";

   }

}