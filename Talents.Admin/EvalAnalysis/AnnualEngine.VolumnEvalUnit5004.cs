...
namespace TheSite.EvalAnalysis
{
	...
	public partial class AnnualEngine
	{
		...
		public class VolumnEvalUnit5004 : VolumnEvalUnit
		{
			...
			private void AnalysisResult(long teacherId, APDBDef db, EvalVolumnResult result, List<EvalVolumnResultItem> items)
			{
				...
				result.AnalysisContent += JiaosPeixKecScore == 0 ? "\"培训课程.开设教师培训课程\"分析： 您还需要参加一门\"微课程\"和一门\"区级研训一体课程\"或\"市级课程\"， 可以获得此项满分。" : (JiaosPeixKecScore + ZhuantJiangzScore + KecZiyKafScore) >= 25 ? "" : analysis;
				...
				List<DeclareAchievement> source, ref string analysis)
			{
				...
				var tempStr = "\"自身发展.项目研究\"分析： 您还需要参加区级以上的\"项目研究\"， 可以获得此项满分。";
				...
				List<DeclareActive> source, ref string analysis)
			{
				...
				var tempStr = "\"培训课程.开设教师培训课程\"分析： 您还需要参加一门\"微课程\"和一门\"区级研训一体课程\"或\"市级课程\"， 可以获得此项满分。";

				double tempScore = 0;
				if (source.Exists(m => m.Dynamic2 == LevelNames.PeixJiangz_Shij) || 
					(source.Exists(m => m.Dynamic2 == LevelNames.PeixJiangz_Weikc) && source.Exists(m => m.Dynamic2 == LevelNames.PeixJiangz_YanxYit)))
				{
					tempScore = 25;
					item.ChooseValue = "A";
				...
				else if (source.Exists(m => m.Dynamic2 == LevelNames.PeixJiangz_Shij || m.Dynamic2 == LevelNames.PeixJiangz_Weikc))
				{
					tempScore = 22.5;
					item.ChooseValue = "B";
					item.ResultValue = "22.5分";
				}
				else
				{
					item.ChooseValue = "C";
