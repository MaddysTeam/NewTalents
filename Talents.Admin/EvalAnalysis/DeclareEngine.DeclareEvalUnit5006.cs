	...
	public partial class DeclareEngine
	{
		...
		public  class DeclareEvalUnit5006 : DeclareEvalUnit
		{
         ...
         private double Analysis_JiaoyJiaox_Gongkk(EvalDeclareResult result, Dictionary<string, EvalDeclareResultItem> items, string choose, string score)
         {
            ...
            {
               case "A1":
                  result.Score += EvalHelper.EnsureScoreInRange(scoreValue, 15.0, 15.0);
                  break;
               case "A2":
                  result.Score += EvalHelper.EnsureScoreInRange(scoreValue, 13.0, 14.9);
                  break;
               case "A3":
                  result.Score += EvalHelper.EnsureScoreInRange(scoreValue, 9.0, 12.9);
                  break;
               case "A4":
                  result.Score += EvalHelper.EnsureScoreInRange(scoreValue, 4.0, 8.9);
                  break;
            }
         ...
         private double Analysis_JiaosPeix_PeixKec(EvalDeclareResult result, Dictionary<string, EvalDeclareResultItem> items, string choose, string score)
         {
            ...
            switch (choose)
            {
               case "A1":
                  ...
                  break;
               case "B1":
                  result.Score += EvalHelper.EnsureScoreInRange(scoreValue, 7.0, 8.9);
                  break;
               case "C1":
