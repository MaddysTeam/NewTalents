...
namespace Util.ThirdParty.WangsuCloud
{
	...
	public class FileUploader
	{
		...
		public const string DOMAIN = "tjcdn.shec.edu.cn";

		static SimpleUpload _simpleuploader;
		...
		public static UploadResult Upload(UploadFile file, bool isOverwirte = true)
		{
			...
			Auth auth = new Auth(new Mac(ACCESS_KEY, SECURITY_KEY));// EnvUtility.EnvAuth();
			Config config = new Config(UPLOAD_DOMAIN, MANAGER_DOMAIN);
			BucketManager bm = new BucketManager(auth, config);
			HttpResult httpResult = bm.Delete(bucket, key);
			...
			try
			{
				...
				httpResult = Simpleuploader.UploadStream(file.Stream, putPolicy, key);

				result.IsSuccess = httpResult.Code == 200;
				result.FileUrl = $"http://{FileUploader.DOMAIN}/{key}";
			}
			catch (Exception e)
			{
