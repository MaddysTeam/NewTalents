	...
	public class AttachmentController : BaseController
	{
		...
		public ActionResult UploadFile(HttpPostedFileBase file)
		{
			...
			try
			{
				...
				var result = Upload(fileStream, filePath, false, file.ContentLength);
		...
		private UploadResult Upload(Stream stream, string filePath, bool isforceClose, long size = 0)
		{
			var result = size < 5242880 ? // is file size larger than 5m then using sliceUpload
		   FileUploader.Upload(new UploadFile
		   {
			   Stream = stream,
			   FileName = filePath
		   }) :

			FileUploader.SliceUpload(new UploadFile
			{
				Stream = stream,
				FileName = filePath
			});
