...
namespace TheSite.EvalAnalysis
{
	...
	public partial class AnnualEngine
	{
		...
		public class VolumnEvalUnit5006 : VolumnEvalUnit
		{
            ...
            ZisFaz_ZiwYanx(items, result, daList.FindAll(m => m.ActiveKey == DeclareKeys.ZisFaz_ZiwYanx));
				...
				analysis = "";
				var JiaosPeixKecScore = ZisFaz_PeixJiangz_JiaosPeixKec(items, daList.FindAll(m => m.ActiveKey == DeclareKeys.ZisFaz_PeixJiangz_JiaosPeixKec), ref analysis);
				...
				var KecZiyKafScore = ZhidJians_TesHuodKaiz_KecZiyKaif(items, daList.FindAll(m => m.ActiveKey == DeclareKeys.ZisFaz_PeixJiangz_KecZiyKaif && m.Dynamic9 == PicklistHelper.TesHuodKaizType.GetItemId(TesHuodKaizKeys.KecZiyl)), ref analysis);
				result.Score += EvalHelper.GetScore(25, JiaosPeixKecScore, ZhuantJiangzScore, KecZiyKafScore);
				result.AnalysisContent += (JiaosPeixKecScore + ZhuantJiangzScore + KecZiyKafScore) >= 25 ? "" : analysis;
				...
				List<DeclareActive> source)
			{
				...
				if (cnt < 2)
				{
					result.AnalysisContent += string.Format("\"自身发展.读书活动\"分析： 您还需要填写{0}篇\"自身发展.读书活动\"， 可以获得此项满分。", 2 - cnt);
				}
			}
				...
				List<DeclareActive> source, ref string analysis)
			{
				...
				double tempScore = 0;
				var tempStr = "\"自身发展.开展示课\"分析： 您需要参加的\"教学公开课\"或\"研讨课\"， 可以获得此项满分。";
				...
				List<DeclareActive> source, ref string analysis)
			{
				...
				items.Add(item);
				var tempStr = "\"自身发展.担任评委工作\"分析: 您需要参加的\"担任评委工作\", 可以获得此项满分。";
				...
				List<DeclareActive> source, ref string analysis)
			{
				...
				items.Add(item);
				var tempStr = "\"自身发展.评比获奖\"分析： 您还需要参加校级以上的\"评比获奖\"， 可以获得此项满分。";
				...
				List<DeclareAchievement> source, ref string analysis)
			{
				...
				items.Add(item);
				var tempStr = "\"自身发展.发表论文\"分析： 您还需要参加校级以上的\"发表论文\"， 可以获得此项满分。";
				...
				List<DeclareAchievement> source, ref string analysis)
			{
				...
				var tempStr = "\"自身发展.立项课题\"分析： 您还需要参加校级以上的\"立项课题\"， 可以获得此项满分。";
				...
				List<DeclareAchievement> source, ref string analysis)
			{
				...
				var tempStr = "\"自身发展.项目研究\"分析： 您还需要参加\"项目研究\"， 可以获得此项满分。";
				...
				List<DeclareActive> source, ref string analysis)
			{
				...
				var tempStr = "\"培训课程.开设教师培训课程\"分析： 您还需要参加一门\"微课程\"或一门\"区级研训一体课程\"或\"市级课程\"， 可以获得此项满分。";

				double tempScore = 0;
				if (source.Exists(m => m.Dynamic2 == LevelNames.PeixJiangz_Shij ||
					m.Dynamic2 == LevelNames.PeixJiangz_Weikc || m.Dynamic2 == LevelNames.PeixJiangz_YanxYit))
				{
					tempScore = 25;
					item.ChooseValue = "A";
				...
				List<DeclareActive> source, ref string analysis)
			{
				...
				var tempStr = "\"培训课程.讲座、报告\"分析： 您需要参加\"讲座、报告\"， 可以获得此项满分。";
				...
				List<DeclareActive> source, ref string analysis)
			{
				...
				var tempStr = "\"培训课程.课程资源开发\"分析： 您需要参加\"课程资源开发\"， 可以获得此项满分。";
			...
			private void XueyShul(int MemberCount, EvalVolumnResult result, List<EvalVolumnResultItem> items)
			{
				...
				if (MemberCount < 1)
				{
					result.AnalysisContent += string.Format("\"带教教师.学员数量\"分析： 你还需带{0}学员， 可以获得此项满分。", 1 - MemberCount);
				}
			}
			...
			private void DaijChengg(EvalVolumnResult result, List<EvalVolumnResultItem> items, List<DeclareActiveDataModel> source, List<DeclareAchievementDataModel> source1, int memberCount)
			{
				...
				var JiaoyXuekKeyChenggScore = JiaoyXuekKeyChengg(items, source1, memberCount, ref analysis);

				var JiaoyJiaoxPingbScore = JiaoyJiaoxPingb(items, source.FindAll(m => (m.ActiveKey == DeclareKeys.ZisFaz_JiaoxHuod_JiaoxPingb)), memberCount, ref analysis);
