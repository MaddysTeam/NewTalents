...
namespace TheSite.EvalAnalysis
{
	...
	public static class EngineManager
	{
		...
		public static TeamEvalResultItem GetTeamEvalResult(Dictionary<string, TeamEvalResultItem> exist, string key, string defaultValue = "A")
		{
			var defaultResult = new TeamEvalResultItem { ChooseValue = defaultValue, ResultValue = string.Empty };
			if (exist == null)
				return defaultResult;
			if (exist.ContainsKey(key))
				return exist[key];
		...
		public static string GetTeamEvalChooseValue(Dictionary<string,TeamEvalResultItem> exist,string key,string defaultValue="A")
		{
			if (exist == null)
				return defaultValue;
			if (exist.ContainsKey(key))
		...
		public static string GetTeamEvalResultValue(List<TeamEvalResultItem> exist, string key, string defaultValue = "0分")
		{
			if (exist == null)
				return defaultValue;
			if (exist.Exists(m => m.EvalItemKey == key))
